...
public:
	...
	//used when mesh's textures vector is empty.
	vector<Texture> extra_textures;

	// constructor, expects a filepath to a 3D model.
	Model(string const& path, bool skipTextures, bool gamma = false) : gammaCorrection(gamma)
	{
		this->skipTextures = skipTextures;
		loadModel(path);
	}
...
// 		}
		...
		{   // if texture hasn't been loaded already, load it
			...
			texture.path = str;
			extra_textures.push_back(texture);
		}
	}

	void UseExtraTexture(Shader& shader)
	{
		...
		for (unsigned int i = 0; i < extra_textures.size(); i++)
		{
			...
			string number;
			string name = extra_textures[i].type;
			...
			else if (name == "texture_normalcamera")
				number = std::to_string(normalcameraNr++); // transfer unsigned int to stream
			else if (name == "texture_metalness")
				number = std::to_string(metalnessNr++); // transfer unsigned int to stream
			...
			glBindTexture(GL_TEXTURE_2D, extra_textures[i].id);
		}

		//mesh's Draw() will do this;
